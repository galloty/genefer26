...
public:
	explicit mpz_vec() { for (size_t j = 0; j < VSIZE; ++j) mpz_init(_z[j]); }
	virtual ~mpz_vec() { for (size_t j = 0; j < VSIZE; ++j) mpz_clear(_z[j]); }

	const mpz_t & operator[](const size_t i) const { return _z[i]; }
	mpz_t & operator[](const size_t i) { return _z[i]; }

	void set_ui(const bvec & n)
	{
		mpz_t * const z = _z;
	...
	void set_exponent(const bvec & b, const int n)
	{
		mpz_t * const z = _z;
	...
	void set_GL_residue(const mpz_vec & exponent, const int B_GL)
	{
		...
		for (size_t j = 0; j < VSIZE; ++j) mpz_set_ui(z[j], 0);
	...
	void set_PL_residue(const mpz_vec & exponent, const int B_PL, const mpz_vec * const w, const size_t L)
	{
		...
		for (size_t j = 0; j < VSIZE; ++j) mpz_set_ui(_z[j], 0);
	...
	void mul_ui(const mpz_vec & rhs, const bvec & n)
	{
		mpz_t * const z = _z;
	...
	void mul_2exp(const mpz_vec & rhs, const unsigned long int e)
	{
		...
		for (size_t j = 0; j < VSIZE; ++j) mpz_mul_2exp(z[j], rz[j], e);
	}
	...
	size_t get_max_size() const
	{
		...
		size_t max_index = 0;
		for (size_t j = 0; j < VSIZE; ++j) max_index = std::max(max_index, mpz_sizeinbase(z[j], 2));
		return max_index;
	}
	...
	uint32_t get_bit_mask(const int i) const
	{
		...
		uint32_t mask = 0;
		for (size_t j = 0; j < VSIZE; ++j) mask |= ((mpz_tstbit(z[j], mp_bitcnt_t(i)) != 0) ? 1u : 0u) << j;
		return mask;
	}
