...
template<> \
transform<SIZE> * transform<SIZE>::create_ocl_avx512(const b_vec<SIZE / 8> & b, const int n, const size_t num_regs, const size_t device, \
...
transform<VSIZE> * transform<VSIZE>::create_ocl_avx512(const b_vec<VSIZE / 8> & b, const int n, const size_t num_regs, const size_t device,
											const bool is_boinc, const bool get_boinc_ids, int _boinc_argc, char ** _boinc_argv)
{
	return nullptr;
}
