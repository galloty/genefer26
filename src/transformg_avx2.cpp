								  ...
								  const bool is_boinc, const bool get_boinc_ids, int _boinc_argc, char ** _boinc_argv)
{
	...
	if (b_max <= 1000000000)
	{
		ptransform = new arch_g_avx2_namespace::transformGPU<8, 2, false>(b, n, num_regs, device, is_boinc, boinc_platform_id, boinc_device_id);
	}
	else
	{
		ptransform = new arch_g_avx2_namespace::transformGPU<8, 2, true>(b, n, num_regs, device, is_boinc, boinc_platform_id, boinc_device_id);
	}
