								  ...
								  const bool is_boinc, const bool get_boinc_ids, int _boinc_argc, char ** _boinc_argv)
{
	...
	if (b_max <= 1000000000)
	{
		ptransform = arch_g_avx2_namespace::create_transformGPU<8, false>(b, n, num_regs, device, is_boinc, boinc_platform_id, boinc_device_id);
	}
	else
	{
		ptransform = arch_g_avx2_namespace::create_transformGPU<8, true>(b, n, num_regs, device, is_boinc, boinc_platform_id, boinc_device_id);
	}
